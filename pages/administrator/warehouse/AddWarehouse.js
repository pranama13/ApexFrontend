...
export default function AddWarehouse({ fetchItems, companies }) {
  const [open, setOpen] = useState(false);
  const [warehouseCode, setWarehouseCode] = useState(null);
  const handleOpen = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}/DocumentSequence/GetNextDocumentNumber?documentType=12`,
        {
          method: "POST",
          headers: {
      ...
      setWarehouseCode(result.result);
    } catch (err) {
      //
    }
    setOpen(true);
  ...
  const handleSubmit = (values) => {
    ...
    fetch(`${BASE_URL}/Warehouse/CreateWarehouse`, {
      method: "POST",
