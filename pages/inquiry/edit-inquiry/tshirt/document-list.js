...
export default function DocumentListTShirt({inquiry}) {
  ...
  const fetchNeckType = async (inquiryId, optionId, windowType) => {
    try {
      const response = await fetch(
      ...
      fetchNeckDetails(data.result.necKTypes, inquiryId, optionId, windowType);
    } catch (error) {
      console.error("Error fetching Neck Body List:", error);
    }
  };

  const fetchNeckDetails = async (neck, inquiryId, optionId, windowType) => {
    try {
      const response = await fetch(
        `${BASE_URL}/InquiryNeck/GetAllNeckTypes?InquiryID=${inquiryId}&OptionId=${optionId}&WindowType=${windowType}&necKTypes=${neck}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  useEffect(() => {
    if (inquiry) {
      ...
      fetchNeckBodyList(inquiry.inquiryId, inquiry.optionId, inquiry.windowType);
      fetchNeckType(inquiry.inquiryId, inquiry.optionId, inquiry.windowType);
    }
  }, [inquiry]);
      ...
      <Grid container>
        <Grid item xs={12} lg={5} pr={1}>
          ...
          <Box>
            ...
            <Box display="flex" flexWrap="wrap" my={1} gap={1}>
              {isContrastSelected && <Chip size="small" label="Contrast" color="primary" />}
              {isAHPiePinSelected && <Chip size="small" label="A/H Pie Pin" color="primary" />}
              {isCuffPiePinSelected && <Chip size="small" label="Cuff Pie Pin" color="primary" />}
              {isBottomDHEMSelected && <Chip size="small" label="Bottom D / Hem" color="primary" />}
