...
export default function Sleeve() {
  ...
  const generateLink = (neckValue) => {
    switch (neckValue) {
      case "1":
        return `/production/ongoing/tshirt/polo-neck/?inqType=${inqType}`;
      case "2":
        return `/production/ongoing/tshirt/crew-neck/?inqType=${inqType}`;
      case "3":
        return `/production/ongoing/tshirt/v-neck/?inqType=${inqType}`;
      default:
        return `/production/ongoing/tshirt/neck/?inqType=${inqType}`;
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    router.push(
      `/production/ongoing/tshirt/document-panel/?inqType=${inqType}`
    );
  };
