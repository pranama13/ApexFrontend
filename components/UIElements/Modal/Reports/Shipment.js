...
const ShipmentReport = ({ shipment }) => {
  const currentDate = new Date().toISOString().split("T")[0];
  const [open, setOpen] = useState(false);
  const [items] = useState(shipment.shipmentNoteLineDetails);
  ...
  const UploadPDF = async () => {
    ...
    try {
      ...
      formData.append("file", blob, `${shipment.documentNo}.pdf`);
      formData.append("fileName", `${shipment.documentNo}.pdf`);
      formData.append("storePath", "Shipment");
      ...
      <Page size="A4" style={ShipmentStyles.page}>
        <View style={ShipmentStyles.section}>
          <Image
            ...
            src="/images/quotation/shipment.jpg"
          />
          <Text style={ShipmentStyles.date}>
            Date : {formatDate(currentDate)}
          </Text>
          <Text style={ShipmentStyles.invoiceNo}>
            Shipment No : {shipment.documentNo}
          </Text>
          <Text style={ShipmentStyles.add}>Bill To</Text>
          <Text style={ShipmentStyles.add}>{shipment.supplierName}</Text>
          <Text style={ShipmentStyles.add}>{""}</Text>
          <Text style={ShipmentStyles.add}>{""}</Text>
          <Text style={ShipmentStyles.add}>{""}</Text>
            ...
            {items.length === 0 ? (
              ...
              items.map((item, index) => (
                <View key={index} style={ShipmentStyles.tableRow}>
                  ...
                  <Text style={ShipmentStyles.tableCell}>
                    {formatCurrency(item.freightDutyCost ?? 0)}
                  </Text>
                  <Text
            ...
            >
              <Box>
                <ShareReports
                  ...
                  item={shipment}
                  type={1}
                  mobile={shipment.supplierMobileNo}
